...
def create_collada_model(polygon, output_path):
    ...
    positions = [f"{x} {y} {z}" for x, y, z in polygon]
    positions_text = " ".join(positions)
    source = ET.SubElement(mesh, 'source', id='position_source')
    float_array = ET.SubElement(source, 'float_array', id='position_array', count=str(len(polygon) * 3))
    float_array.text = positions_text

    technique_common = ET.SubElement(source, 'technique_common')
    accessor = ET.SubElement(technique_common, 'accessor', source='#position_array', count=str(len(polygon)), stride='3')
    ...
    triangles = ET.SubElement(mesh, 'triangles', count=str(len(polygon) - 2))
